...
t_status string_basic_new( t_size size, t_string * p_string )
{
  ...
  if( p_string == OSAPI_STRING_NULL_POINTER || size == 0 )
    { status_iset( OSAPI_MODULE_STRING,__func__, osapi_string_error_params, &st ); return st; }

  errno = 0;
  p_string->ps_location = (char *) calloc(1, size + 1 );  // Always request 1 byte extra to ensure that the '\0' ends the array of chars
...
t_status string_basic_create( const char * p_cString, t_string * p_string )
{
  t_status	st;
  t_size	len;
  ...
  if( p_string == OSAPI_STRING_NULL_POINTER || p_cString == OSAPI_NULL_CHAR_POINTER )
    { status_iset( OSAPI_MODULE_STRING, __func__, osapi_string_error_params, &st ); return st; }

  len = (t_size) strlen( p_cString );

  errno = 0;

  p_string->ps_location = (char *) calloc(1, len + 1 );	// Always request 1 byte extra to ensure that the '\0' ends the array of chars

  if( p_string->ps_location == OSAPI_NULL_CHAR_POINTER )
    { status_eset( OSAPI_MODULE_STRING, __func__, errno, &st ); return st; }

  if( strcpy( p_string->ps_location, p_cString ) != OSAPI_NULL_CHAR_POINTER )
...
t_status string_basic_put( const char * message, t_string * p_string )
{
  ...
  if( p_string->ps_location == OSAPI_NULL_CHAR_POINTER )
    { status_iset( OSAPI_MODULE_STRING, __func__, osapi_string_error_nullPointer, &st ); return st; }

  if( p_string->csize > 0 )
    { status_iset( OSAPI_MODULE_STRING, __func__, osapi_string_error_exists, &st ); return st; }

  len = (t_size) strlen( message ) + 1;
  if( p_string->tsize < len )
    { status_iset( OSAPI_MODULE_STRING, __func__, osapi_string_error_noSpace, &st ); return st; }
...
t_status string_basic_set( const char * message, t_string * p_string )
{
  ...
  if( len > p_string->tsize )
      len = p_string->tsize;	// Limit the size of the copied string
...
t_status string_basic_concat( t_string * ps_str1, t_string * ps_str2, t_string * ps_final )
{
  ...
  if( ps_final->ps_location == ps_str1->ps_location || ps_final->ps_location == ps_str2->ps_location )
    {
      // One of the string is the same as the destination string
      status_iset( OSAPI_MODULE_STRING, __func__, osapi_string_error_sameString, &st );
      return st;
    }
